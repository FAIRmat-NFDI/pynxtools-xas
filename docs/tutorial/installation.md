...
- how to install `pynxtools-xas`
- how to install `pynxtools-xas` together with NOMAD
...
=== "uv"
    ...
    ```bash
    uv pip install git+https://github.com/FAIRmat-NFDI/pynxtools-xas.git
    ```

=== "pip"

    ```bash
    pip install git+https://github.com/FAIRmat-NFDI/pynxtools-xas.git
    ```

### How to install `pynxtools-xas` with NOMAD

To use `pynxtools-xas` with NOMAD, simply install it in the same environment as the `nomad-lab` package. NOMAD will recognize `pynxtools-xas` as a plugin automatically. In addition, NOMAD will install a schema for NeXus application definitions.

## Start using `pynxtools-xas`

That's it! You can now use `pynxtools-xas`!
